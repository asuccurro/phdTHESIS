...
subset of the former with two very restrictive cut applied on it.
...
\loose\ selection & Preselection  \\
                  & $\geq 1~W_{\rm had}$ candidates \\
                  & $\HT>800\gev$ \\
                  & $\pt(b_1) > 160\gev$\\
                  & $\pt(b_2) >80\gev$ \\
                  & $\Delta R(\ell,\nu)<1.2$ \\
\midrule
\tight\  selection & \loose\ selection \\
     	      &  min$\Delta R(\ell,b)>1.4$\\
              &  min$\Delta R(W_{\rm had},b)>1.4$ \\
\bottomrule
\end{tabular}
...
After finding at least one \whad\ candidate, a variable called $\HT$ is defined
...
in Figure~\ref{fig:HTAll}. A cut at $\HT>800\gev$ is chosen as particularly
...
The cuts chosen are $\pt > 160\gev$ and $\pt >80\gev$ for $b_1$ and $b_2$ respectively.
...
required to satisfy $\Delta R(\ell,\nu)<1.2$. What was described up
to now are the requirements that lead to the \loose\ selection, as was
...
$\min(\Delta R(W_{\rm had}, b_{1,2}))$ (Figure~\ref{fig:DRWb}) are defined as
 $\min(\Delta R(\ell, b_{1,2}))>1.4$ and $\min(\Delta R(W_{\rm had}, b_{1,2}))>1.4$.  
...
$t\bar{t}$ yield in the \loose\ channel, exploiting the well defined 
sideband region at low $M_{reco}$ that is dominated by the \ttbar\ background
contribution. No profiled parameters are used in the \tight\ analysis.
...
in the $m_{\rm reco}$ distribution for $t\bar{t}$ and ``non-$t\bar{t}$" backgrounds in the \tight\ analysis,
when comparing the varied JES templates with the nominal one. To avoid injecting
noise in the statistical analysis, the same shape systematic as for the \loose\ analysis,
which is smoother owing to the higher-statistics MC templates, is applied to the \tight\ 
...
the estimated $W$+jets normalisation for events satisfying the \tight\ selection.
...
between data and MC in a number of control regions, often defined starting from the \loose\ selection but with one of
...
{\sc MC@NLO} fully simulated sample. The resulting uncertainty on the $t\bar{t}$ yield after the \tight\ selection is 48\%, and constitutes the
main source of uncertainty on the $t\bar{t}$ background. The corresponding uncertainty after the \loose\ selection is 16\%. The significant
increase in the uncertainty for the \tight\ selection primarily results from the increased jet multiplicity in {\sc PowHeg} compared to {\sc MC@NLO}, 
...
and propagated to the {\sc MC@NLO} fully-simulated sample, resulting in an uncertainty on the $t\bar{t}$ yield after the \tight\ selection of 8.8\%.
...
{\sc MC@NLO} fully simulated sample, resulting in an uncertainty on the $t\bar{t}$ yield after the \tight\ selection of 25\%. 
