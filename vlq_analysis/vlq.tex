...
optimized for events with high branching ratio to $Wb$,
...
The other search in the single lepton channel,
optimized for events with high branching ratio to $Ht$, considers final states with high
...
Again a top quark mass of $172.5\gev$ is assumed, and normalization to the
...
\subsection{$W$+jets background normalization}\label{sec:Wjetsnorm}
 
For the $W$+jets background, a normalization from data for the shapes 
...
are obtained as $K_{x,i} = F^{\rm data}_{x,i}/F^{\rm MC}_{x,i}$, where
$F^{\rm MC}_{x,i}$ are the flavor fractions from simulation.
The scale factors obtained for $W+2$jets events 
are then applied to  $W$+jets events
with at least 4 jets by renormalizing to unity the sum of the
flavor fractions $K_{x,2}F^{\rm MC}_{x,4}$.
...
\subsection{Data-driven backgrounds normalization and background modeling}
\label{sec:syst_norm}
